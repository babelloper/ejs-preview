...
(function ($, undefined) {
  $.extend($.fn, {
    splitLayout: function splitLayout(opts) {
    ...
    search: function search(opts) {
      ...
      var searchIt = function searchIt(res) {
        ...
        var matchedResult = [];
        $notFound.remove();
        $list.removeClass([opts.visibilityClass, opts.ancestorClass, opts.lastVisibleClass].join(' ')).find('mark').contents().unwrap();
        if (!keyword) {
        ...
        var loop = function loop($elems) {
          ...
          $elems.each(function () {
            ...
            if (regexp.test(text) || href && ~href.indexOf(urlKeyword)) {
              $this.html(text.replace(regexp, markedKeyword)).parent('[data-search-level]').addClass(opts.visibilityClass).parents('[data-search-level]').addClass(opts.ancestorClass);
      ...
      var inputTimeout;
      var keyInput = function keyInput() {
        var _this = this;
        if (inputTimeout) {
    ...
    pageSwitcher: function pageSwitcher() {
      var $menu = this.eq(0);
      ...
      var selectedIndex = 0;
      var pageName = function pageName() {
        return window.location.hash.slice(1);
      };
      var historyState = function historyState(method, $elem) {
        var token = $elem.data('token');
        history[method]({
          elem: token
        }, null, '#' + token);
      };
      var menuHighlightChanger = function menuHighlightChanger(i) {
        $menuItems.filter('.current').removeClass('current').end().eq(i).addClass('current');
      };
      var menuSlider = function menuSlider($elem) {
      ...
      var initialSelection = function initialSelection() {
        var page = pageName();
        var $selectedMenu;
        if (page) {
    ...
    toggleLayout: function toggleLayout() {
      ...
      var statusChange = function statusChange(method) {
      ...
      var toggleIt = function toggleIt(e) {
